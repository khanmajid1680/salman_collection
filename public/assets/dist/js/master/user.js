        ...
        if(status){
            if(flag == 1){
                if(data){
                    ...
                    $(`#${term}_role`).val(data[0][`${term}_role`])
                    id == 1 && $(`#${term}_role`).prop('disabled', true)
...
const user_popup = (id, field = undefined) => {
    let path = `master/user/get_data_for_user`
    ajaxCall('GET',path,'','JSON',resp=>{
        const {status, flag, data, msg} = resp;
        if(status){
            if(flag == 1){
                    ...
                    <div class="row">
                        <div class="col-12">
                            <div class="d-flex flex-wrap form-group floating-form">
                                ...
                                <div class="col-6 floating-label">
                                    <input type="text" name="${term}_name" class="form-control floating-input" id="${term}_name" onkeyup="validate_length(this, 'Length alteast 4 character.', 0, 4)" placeholder=" " autocomplete="off" style="text-transform: none;" />   
                                    <label for="inputEmail3">USERNAME <span style="color:red">*</span><span class="font-italic" style="font-size:12px;">MIN. LENGTH(4)</span></label>
                                    <small class="form-text text-muted helper-text" id="${term}_name_msg"></small>
                                </div>
                                <div class="col-6 floating-label">
                                    <input type="password" name="${term}_password" class="form-control floating-input" id="${term}_password" onkeyup="validate_length(this, 'Length alteast 4 character.', ${id}, 4)" placeholder=" " autocomplete="off" style="text-transform: none;" />   
                                    <label for="inputEmail3">PASSWORD <span style="color:red">*</span><span class="font-italic" style="font-size:12px;">MIN. LENGTH(4)</span></label>
                                    <small class="form-text text-muted helper-text" id="${term}_password_msg"></small>
                                </div>
                                <div class="col-6 floating-label">
                                    <input type="password" class="form-control floating-input" id="${term}_repassword" onkeyup="validate_password(this, 'user_password', ${id})" placeholder=" " autocomplete="off" style="text-transform: none;" />   
                                    <label for="inputEmail3">RETYPE - PASSWORD <span style="color:red">*</span></label>
                                    <small class="form-text text-muted helper-text" id="${term}_repassword_msg"></small>
                                </div>
                                <div class="col-6 floating-label">
                                    <p for="inputEmail3">ROLE <span style="color:red">*</span></p>
                                    <select class="form-control floating-select" id="${term}_role" name="${term}_role">
                                        ${roles}
                                    </select>
                                    <small class="form-text text-muted helper-text" id="${term}_role_msg"></small>
                                </div>
                                <div class="col-6 floating-label">
...
const remove_user_notifier = term => {
    ...
    notifier(`${term}_name`)
    notifier(`${term}_password`)
    notifier(`${term}_repassword`)
    notifier(`${term}_role`)
...
const add_update_user = (id, field) =>{
    ...
    if($(`#${term}_name`).val() == ''){
        ...
        if($(`#${term}_name`).val().length <= 4){
          notifier(`${term}_name`, 'Length alteast 4 character.')
            check = false;  
        }
    }
    if(id == 0){
        if($(`#${term}_password`).val() == ''){
            notifier(`${term}_password`, 'Required')
            check = false;
        }else{
            if($(`#${term}_password`).val().length <= 4){
              notifier(`${term}_password`, 'Length alteast 4 character.')
                check = false;  
            }
        }
        if($(`#${term}_repassword`).val() == ''){
            notifier(`${term}_repassword`, 'Required')
            check = false;
        }else{
            if($(`#${term}_password`).val() != $(`#${term}_repassword`).val()){
              notifier(`${term}_repassword`, 'Password mismatch.')
                check = false;  
            }
        }
    }
    if($(`#${term}_role`).val() == ''){
        notifier(`${term}_role`, 'Required')
        check = false;
    }
