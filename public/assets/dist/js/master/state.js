$(document).ready(function(){
    $("#id").select2(select2_default({
        url:`master/state/get_select2`,
        placeholder:'STATE',
    })).on('change', () => trigger_search());
});
const set_state_field = id => {
    const term  = "state";
...
const state_popup = (id, field = undefined) =>{
    const term  = "state";
...
const remove_state_notifier = term => {
    notifier(`${term}_name`)
}
const add_update_state = (id, field) =>{
    event.preventDefault();
    const term  = "state";
    let check   = true;
    remove_state_notifier(term)
    if($(`#${term}_name`).val() == ''){
