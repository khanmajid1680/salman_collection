$(document).ready(function(){
    $("#id").select2(select2_default({
        url:`master/country/get_select2`,
        placeholder:'COUNTRY',
    })).on('change', () => trigger_search());
});
const set_country_field = id => {
    const term  = "country";
...
const country_popup = (id, field = undefined) =>{
    const term  = "country";
    ...
    if(id != 0){
        set_country_field(id);
    }
    $(`#${term}_status`).bootstrapToggle()
}
const remove_country_notifier = term => {
    notifier(`${term}_name`)
}
const add_update_country = (id, field) =>{
    event.preventDefault();
    const term  = "country";
    let check   = true;
    remove_country_notifier(term)
    if($(`#${term}_name`).val() == ''){
