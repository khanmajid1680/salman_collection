...
class User extends CI_Controller{
	...
	public function __construct(){
		...
		$this->load->model('master/Usermdl', 'model');
	...
	public function add_update($id){
		...
		$post_data[$this->term.'_fullname'] 	= trim($post_data[$this->term.'_fullname']);
		$post_data[$this->term.'_name'] 		= trim($post_data[$this->term.'_name']);
		$post_data[$this->term.'_mobile'] 		= trim($post_data[$this->term.'_mobile']);
		$post_data[$this->term.'_email'] 		= trim($post_data[$this->term.'_email']);
		$post_data[$this->term.'_address'] 		= trim($post_data[$this->term.'_address']);
		$post_data[$this->term.'_status'] 		= isset($post_data[$this->term.'_status']);
		$post_data[$this->term.'_updated_by'] 	= $_SESSION['user_id'];
		$temp = $this->db_operations->get_record($this->table, [$this->term.'_id !=' => $id, $this->term.'_name' => $post_data[$this->term.'_name'], $this->term.'_branch_id' => $post_data[$this->term.'_branch_id']]);
		if(!empty($temp)){
			echo json_encode(['status' => true, 'flag' => 2, 'data' => [], 'msg' => 'Username already exist.']);
			return;	
		}
		if($id == 0){
			$post_data[$this->term.'_password'] 	= md5(trim($post_data[$this->term.'_password']));
			...
			if(!empty($post_data[$this->term.'_password'])){
				$post_data[$this->term.'_password'] 	=  md5(trim($post_data[$this->term.'_password']));
			}else{
				unset($post_data[$this->term.'_password']);
			}
		...
		$data['id'] 	= $id;
		$data['name'] 	= strtoupper($post_data[$this->term.'_fullname']);
	...
	public function get_data_for_user(){
		...
		$data['branches'] 	= $this->Branchmdl->get_record(['branch_status' => true], true);
		echo json_encode(['status' => true, 'flag' => 1, 'data' => $data, 'msg' => 'Data fetched successfully.']);
	}
	public function remove($id){
		...
		$data = $this->model->get_record([$this->term.'_id' => $id]);
		if(empty($data)){
			echo json_encode(['status' => true, 'flag' => 2, 'data' => [], 'msg' => ucfirst($this->term).' already deleted.']);
			return;	
		}
		$isExist = $this->model->isExist($id);
		if($isExist){
			echo json_encode(['status' => true, 'flag' => 2, 'data' => [], 'msg' => ucfirst($this->term).' already exist.']);
			return;	
		}
	...
	public function get_select2_user(){
		$json = [];
		$data = $this->model->get_select2_user();
