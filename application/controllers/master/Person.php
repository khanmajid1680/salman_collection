...
class Person extends CI_Controller{
	...
	public function __construct(){
		...
		$this->load->model('master/Personmdl', 'model');
		...
		if(sessionExist()){
			if(isset($_GET['action'])){
				if($_GET['action'] == 'view'){
					...
					$config['base_url'] 	= base_url("master/person?search=true");
					...
					$this->load->view('pages/master/person_master', $record);
				}else{
	...
	public function add_update($id){
		...
		$temp = $this->db_operations->get_record($this->table, [$this->term.'_id !=' => $id, $this->term.'_fullname' => $post_data[$this->term.'_fullname'], $this->term.'_branch_id' => $post_data[$this->term.'_branch_id']]);
		if(!empty($temp)){
			echo json_encode(['status' => true, 'flag' => 2, 'data' => [], 'msg' => 'Sales person already exist.']);
			return;	
		}
		if($id == 0){
