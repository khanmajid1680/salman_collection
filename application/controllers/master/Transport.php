...
class transport extends CI_Controller{
	...
	public function __construct(){
		...
		$this->load->model('master/transportmdl', 'model');
		...
		if(sessionExist()){
			if(isset($_GET['action'])){
				if($_GET['action'] == 'view'){
					...
					$config['base_url'] 	= base_url("master/transport?search=true");
					...
					$this->load->view('pages/master/transport_master', $record);
				}else{
	...
	public function add_update($id){
		...
		$post_data[$this->term.'_gst_no'] 		= trim($post_data[$this->term.'_gst_no']);
		...
		if(!empty($temp)){
			echo json_encode(['status' => true, 'flag' => 2, 'data' => [], 'msg' => 'transport name already exist.']);
			return;	
		}

		$temp = $this->db_operations->get_record($this->table, [$this->term.'_id !=' => $id, $this->term.'_gst_no' => $post_data[$this->term.'_gst_no']]);
		if(!empty($temp)){
			echo json_encode(['status' => true, 'flag' => 2, 'data' => [], 'msg' => 'GSt Number already exist.']);
			return;	
		}
	...
	public function get_select2($fun){
		$json = [];
		$data = $this->model->$fun();
