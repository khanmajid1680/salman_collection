...
class Branch_receipt extends CI_Controller{
	protected $master;
	protected $trans;
	...
	public function __construct(){
		...
		$this->load->model('voucher/BranchReceiptmdl', 'model');
		...
		if(sessionExist()){
			if(isset($_GET['action'])){
				if($_GET['action'] == 'view'){
					...
					$config['base_url'] 	= base_url("voucher/branch_receipt?search=true");
					...
					$this->load->view('pages/voucher/branch_receipt_master', $record);
				}else if($_GET['action'] == 'add'){
					$record = $this->model->get_data_for_add();
					$this->load->view('pages/voucher/branch_receipt_form', $record);
	...
	public function get_branch_balance($id, $constant){
		if(!sessionExist()){
		...
		$data['bal_data'] 		= $this->model->get_branch_balance($temp, $constant);
		$data['bill_amt']		= $this->model->get_outward_balance($id);
		$data['receipt_amt']	= $this->model->get_receipt_balance($id, $constant);
		$data['outward_data'] 	= $this->model->get_data_for_receipt($id, $constant);
		...
		foreach ($post_data['vt_id'] as $key => $value){
			if($post_data['vt_bill_clear'][$key] == 'YES'){
				if($post_data['vt_adjust_amt'][$key] > 0){
					...
					$trans_data['vt_gm_id'] 	 	= 0;
					$trans_data['vt_om_id'] 	 	= $post_data['vt_om_id'][$key];
					...
					$trans_data['vt_return_qty'] 	= $post_data['vt_return_qty'][$key];
					$trans_data['vt_total_amt'] 	= $post_data['vt_total_amt'][$key];
					$trans_data['vt_return_amt'] 	= $post_data['vt_return_amt'][$key];
