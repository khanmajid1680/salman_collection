...
class Payment extends CI_Controller{
	...
	public function __construct(){
		...
		$this->load->model('voucher/Paymentmdl', 'model');
		...
		if(sessionExist()){
			if(isset($_GET['action'])){
				if($_GET['action'] == 'view'){
					...
					$config['base_url'] 	= base_url("voucher/payment?search=true");
					...
					$this->load->view('pages/voucher/payment_master', $record);
				}else if($_GET['action'] == 'add'){
					$record = $this->model->get_data_for_add();
					$this->load->view('pages/voucher/payment_form', $record);
	...
	public function insert_update_trans($post_data, $id){
		foreach ($post_data['vt_id'] as $key => $value){
			if($post_data['vt_bill_clear'][$key] == 'YES'){
				...
				$trans_data['vt_pm_id'] 	 	= isset($post_data['vt_pm_id'][$key]) ? $post_data['vt_pm_id'][$key] : 0;
	...
	public function add_update_payment_mode($post_data, $id){
		...
		foreach ($post_data['ppmt_amt'] as $key => $value){
			if($value > 0){
				...
				if(isset($post_data['ppmt_general_id'][$key]) && !empty($post_data['ppmt_general_id'][$key])){
                    $trans_data['ppmt_general_id'] = $post_data['ppmt_general_id'][$key];
                }else{
                    $payment_mode_data = $this->db_operations->get_record('payment_mode_master', ['payment_mode_id' => $trans_data['ppmt_payment_mode_id']]);
                    if(empty($payment_mode_data)) return ['msg' => '1. payment mode not found.'];

                    if($payment_mode_data[0]['payment_mode_constant'] == 'CASH'){
                        $trans_data['ppmt_general_id'] = isset($post_data['payment_cash_id']) ? $post_data['payment_cash_id'] : 0;
                    }else{
                        $trans_data['ppmt_general_id'] = isset($post_data['payment_bank_id']) ? $post_data['payment_bank_id'] : 0;
                    }
                }
				...
				if(empty($post_data['ppmt_id'][$key])){
					...
					if($this->db_operations->data_update('payment_payment_mode_trans', $trans_data, 'ppmt_id', $post_data['ppmt_id'][$key]) < 1){
	...
	public function get_payment_mode_data($payment_id){
		...
		$data['payment_mode_data'] = $this->model->get_payment_mode_data($payment_id);
		if(empty($data)) {
	...
	public function get_party($id){
		...
		$data = $this->db_operations->get_record('account_master', ['account_id' => $id, 'account_status' => true]);
		echo json_encode(['status' => true, 'flag' => 1, 'data' => $data, 'msg' => 'Data fetched successfully.']);
		return;
	}
