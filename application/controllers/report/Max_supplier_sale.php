...
class Max_supplier_sale extends CI_Controller{
	...
	public function __construct(){
		...
		$this->load->model('report/Max_supplier_salemdl', 'model');
	...
	public function index(){	
		if(sessionExist()){
			...
			if(isset($_GET['submit']) && !empty($_GET['submit']) && $_GET['submit'] == 'PDF'){
				$this->load->view('pdfs/report/max_supplier_sale', $record);
			}else{
				$record['total_rows']	= !empty($record['data']['data']) ? count($record['data']['data']) : 0;
				$this->load->view('pages/report/max_supplier_sale', $record);
			}
		}else{
