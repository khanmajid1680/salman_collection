...
class Purchase_return_summary extends CI_Controller{
	...
	public function __construct(){
		...
		$this->load->model('report/Purchase_return_summarymdl', 'model');
	...
	public function index(){	
		if(sessionExist()){
			...
			if(isset($_GET['submit']) && !empty($_GET['submit']) && $_GET['submit'] == 'PDF'){
				$this->load->view('pdfs/report/purchase_return_summary', $record);
			}else{
				$record['total_rows']	= !empty($record['data']['data']) ? count($record['data']['data']) : 0;
				$this->load->view('pages/report/purchase_return_summary', $record);
			}
		}else{
