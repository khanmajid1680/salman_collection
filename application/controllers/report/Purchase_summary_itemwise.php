...
class Purchase_summary_itemwise extends CI_Controller{
	...
	public function __construct(){
		...
		$this->load->model('report/Purchase_summary_itemwisemdl', 'model');
	...
	public function index(){	
		if(sessionExist()){
			...
			if(isset($_GET['submit']) && !empty($_GET['submit']) && $_GET['submit'] == 'PDF'){
				$this->load->view('pdfs/report/purchase_summary_itemwise', $record);
			}else{
				$record['total_rows']	= !empty($record['data']['data']) ? count($record['data']['data']) : 0;
				$this->load->view('pages/report/purchase_summary_itemwise', $record);
			}
		}else{
	...
	public function excel(){	
		if(sessionExist()){
			...
			$this->excel->getActiveSheet()->setTitle("PURCHASE SUMMARY(ITEMWISE)");
			$this->excel->getActiveSheet()->setCellValue('A'.$line_no, "PURCHASE SUMMARY(ITEMWISE)");			 
			...
			$this->excel->getActiveSheet()->getStyle('N'.$line_no.":P".$line_no)->getFont()->setBold( true );	
			 ...
			 if(!empty($record['data']['data'])){ 
				 ...
				 $this->excel->getActiveSheet()->SetCellValue('S'.$line_no, 'SELLING PRICE');
					
				$this->excel->getActiveSheet()->getColumnDimension('T')->setAutoSize(true);
				 $this->excel->getActiveSheet()->getStyle('T'.$line_no)->getFont()->setBold(true);	
				 $this->excel->getActiveSheet()->SetCellValue('T'.$line_no, 'MRP');	 

				$this->excel->getActiveSheet()->getColumnDimension('U')->setAutoSize(true);
				$this->excel->getActiveSheet()->getStyle('U'.$line_no)->getFont()->setBold(true);	
				$this->excel->getActiveSheet()->SetCellValue('U'.$line_no, 'TOKEN AMT');	 	 

				 $line_no++;
				 foreach ($record['data']['data'] as $key => $value) {
				 	
					 $this->excel->getActiveSheet()->getColumnDimension('A')->setAutoSize(true);
					 $this->excel->getActiveSheet()->SetCellValue('A'.$line_no, $value['pm_bill_no']);
	 
					 $this->excel->getActiveSheet()->getColumnDimension('B')->setAutoSize(true);
					 $this->excel->getActiveSheet()->SetCellValue('B'.$line_no, date('d-m-Y', strtotime($value['pm_bill_date'])));
					 ...
					 $this->excel->getActiveSheet()->getColumnDimension('G')->setAutoSize(true);
					 $this->excel->getActiveSheet()->SetCellValue('G'.$line_no, $value['pt_qty']);
	 
					 $this->excel->getActiveSheet()->getColumnDimension('H')->setAutoSize(true);
					 $this->excel->getActiveSheet()->SetCellValue('H'.$line_no, $value['pt_rate']);
	 
					 $this->excel->getActiveSheet()->getColumnDimension('I')->setAutoSize(true);
					 $this->excel->getActiveSheet()->SetCellValue('I'.$line_no, $value['pt_sub_total']);
	 
					 $this->excel->getActiveSheet()->getColumnDimension('J')->setAutoSize(true);
					 $this->excel->getActiveSheet()->SetCellValue('J'.$line_no, $value['pt_disc_amt']);
	 
					 $this->excel->getActiveSheet()->getColumnDimension('K')->setAutoSize(true);
					 $this->excel->getActiveSheet()->SetCellValue('K'.$line_no, $value['pt_taxable_amt']);
	 
					 $this->excel->getActiveSheet()->getColumnDimension('L')->setAutoSize(true); 
					 $this->excel->getActiveSheet()->SetCellValue('L'.$line_no, $value['pt_sgst_per']);

					 $this->excel->getActiveSheet()->getColumnDimension('M')->setAutoSize(true); 
					 $this->excel->getActiveSheet()->SetCellValue('M'.$line_no, $value['pt_sgst_amt']);

					 $this->excel->getActiveSheet()->getColumnDimension('N')->setAutoSize(true); 
					 $this->excel->getActiveSheet()->SetCellValue('N'.$line_no, $value['pt_cgst_per']);

					 $this->excel->getActiveSheet()->getColumnDimension('O')->setAutoSize(true); 
					 $this->excel->getActiveSheet()->SetCellValue('O'.$line_no, $value['pt_cgst_amt']);

					 $this->excel->getActiveSheet()->getColumnDimension('P')->setAutoSize(true); 
					 $this->excel->getActiveSheet()->SetCellValue('P'.$line_no, $value['pt_igst_per']);

					 $this->excel->getActiveSheet()->getColumnDimension('Q')->setAutoSize(true); 
					 $this->excel->getActiveSheet()->SetCellValue('Q'.$line_no, $value['pt_igst_amt']);

					 $this->excel->getActiveSheet()->getColumnDimension('R')->setAutoSize(true); 
					 $this->excel->getActiveSheet()->SetCellValue('R'.$line_no, $value['pt_sub_total_amt']);

					 $this->excel->getActiveSheet()->getColumnDimension('S')->setAutoSize(true); 
					 $this->excel->getActiveSheet()->SetCellValue('S'.$line_no, $value['pt_sp_amt']);

					 $this->excel->getActiveSheet()->getColumnDimension('T')->setAutoSize(true); 
					 $this->excel->getActiveSheet()->SetCellValue('T'.$line_no, $value['pt_mrp']);

					 $this->excel->getActiveSheet()->getColumnDimension('U')->setAutoSize(true); 
					 $this->excel->getActiveSheet()->SetCellValue('U'.$line_no, $value['pt_token_amt']);
				 ...
				 $this->excel->getActiveSheet()->getColumnDimension('U')->setAutoSize(true);
				 $this->excel->getActiveSheet()->getStyle('U'.$line_no)->getFont()->setBold(true);	
				 $this->excel->getActiveSheet()->SetCellValue('U'.$line_no, $record['data']['totals']['token_amt']['total_amt']);
				
			 }

			 $filename='Purchase_summary_itemwise_'.time().'.xlsx';
