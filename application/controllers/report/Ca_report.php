...
class ca_report extends CI_Controller{
	...
	public function __construct(){
		...
		$this->load->model('report/ca_reportmdl', 'model');
	...
	public function index(){	
		if(sessionExist()){
			...
			if(isset($_GET['submit']) && !empty($_GET['submit']) && $_GET['submit'] == 'PDF'){
				$this->load->view('pdfs/report/ca_report', $record);
			}else{
				$record['total_rows']	= !empty($record['data']['data']) ? count($record['data']['data']) : 0;
				$this->load->view('pages/report/ca_report', $record);
			}
		}else{
