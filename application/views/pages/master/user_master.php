...
<section class="container-fluid sticky_top">
	<form class="form-horizontal" id="search_form" action="<?php echo base_url('master/user?action=view')?>" method="get">
		<div class="d-flex justify-content-between">
			<nav aria-label="breadcrumb">
			  <ol class="breadcrumb">
			    <li class="breadcrumb-item"><a href="<?php echo base_url('master/'.$term.'?action=view'); ?>">MASTER</a></li>
			    <li class="breadcrumb-item active" aria-current="page">
			    	<?php echo strtoupper($term); ?>(<span id="count_reload"><i id="total_rows"><?php echo $total_rows;?></i></span>)
			    </li>
			    <li class="breadcrumb-item" aria-current="add-page">
			    	<a type="button" class="btn btn-sm btn-primary" onclick="user_popup(0)" data-toggle="tooltip" data-placement="bottom" title="ADD NEW"><i class="text-success fa fa-plus"></i></a>
			    </li>
			    ...
			    <li class="breadcrumb-item" aria-current="refresh-page">
			    	<button type="button" class="btn btn-sm btn-primary" onclick="redirectPage('master/user?action=view')" data-toggle="tooltip" data-placement="bottom" title="REFRESH"><i class="text-info fa fa-undo"></i></button>
			    </li>
		...
		<div class="row collapse mt-2 <?php echo empty($search_status) ? '' : 'show'  ?>" id="search_box">
			<div class="d-flex flex-wrap justify-content-center floating-form">
				<div class="col-6 col-sm-3 col-md-3 col-lg-3 floating-label">
					<?php if(isset($data['search']['id'])): ?><p>USER - MOBILE</p><?php endif; ?>
				...
				<div class="col-6 col-sm-3 col-md-3 col-lg-3 floating-label">
					<?php if(isset($data['search']['role'])): ?><p>ROLE</p><?php endif; ?>
					<select class="form-control floating-select" id="role" name="role">
	                    <?php if(isset($data['search']['role']) && !empty($data['search']['role'])): ?>
	                        <option value="<?php echo $data['search']['role']['value']; ?>" selected>
	                            <?php echo $data['search']['role']['text']; ?> 
	                        </option>
	                    <?php endif; ?>
	                </select>
				</div>
				...
				<tbody id="table_tbody">
					...
					?>
								...
								<tr>
									...
									<td width="5%">
										<a type="button" class="btn btn-sm btn-primary" onclick="user_popup(<?php echo $value['id']; ?>)">
...
<?php $this->load->view('templates/footer'); ?>
		<script src="<?php echo assets('dist/js/master/user.js')?>"></script>
	</body>
</html>