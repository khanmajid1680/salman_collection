...
<script> 
    let link 	= "report";
    let sub_link= "purchase_return_summary";
</script>
<section class="container-fluid sticky_top">
	<form class="form-horizontal" id="search_form" action="<?php echo base_url('report/purchase_return_summary?action=view')?>" method="get">
		<div class="d-flex justify-content-between">
			<nav aria-label="breadcrumb">
			  <ol class="breadcrumb">
			    <li class="breadcrumb-item"><a href="<?php echo base_url('report/purchase_return_summary?action=view'); ?>">REPORT</a></li>
			    <li class="breadcrumb-item active" aria-current="page">
			    	PURCHASE RETURN SUMMARY(<span><i><?php echo $total_rows;?></i></span>)
			    </li>
			    ...
			    <li class="breadcrumb-item" aria-current="refresh-page">
			    	<a type="button" class="btn btn-sm btn-primary" onclick="redirectPage('report/purchase_return_summary?action=view')" data-toggle="tooltip" data-placement="bottom" title="REFRESH"><i class="text-info fa fa-undo"></i></a>
			    </li>
			    <li class="breadcrumb-item" aria-current="print-page">
			    	<?php if(!empty($data['data'])): ?>
			    		<a target="_blank" type="button" class="btn btn-sm btn-primary mr-2" data-toggle="tooltip" data-placement="bottom" title="PDF" href="<?php echo base_url("report/purchase_return_summary?submit=PDF&$url"); ?>">
		...
		<div class="row collapse mt-2 <?php echo empty($search_status) ? '' : 'show'  ?>" id="search_box">
			<div class="d-flex flex-wrap justify-content-center floating-form">
				<div class="col-6 col-sm-6 col-md-3 col-lg-2 floating-label">
					<?php if(isset($data['search']['prm_entry_no'])): ?><p>ENTRY NO</p><?php endif; ?>
					<select class="form-control floating-select" id="prm_entry_no" name="prm_entry_no">
                    	<?php if(isset($data['search']['prm_entry_no']) && !empty($data['search']['prm_entry_no'])): ?>
                        	<option value="<?php echo $data['search']['prm_entry_no']['value']; ?>" selected>
                            	<?php echo $data['search']['prm_entry_no']['text']; ?> 
                        	</option>
                    	<?php endif; ?>
                	</select>
				</div>
				...
				<div class="col-6 col-sm-4 col-md-3 col-lg-2 floating-label">
					<?php if(isset($data['search']['prm_acc_id'])): ?><p>SUPPLIER</p><?php endif; ?>
					<select class="form-control floating-select" id="prm_acc_id" name="prm_acc_id">
                    	<?php if(isset($data['search']['prm_acc_id']) && !empty($data['search']['prm_acc_id'])): ?>
                        	<option value="<?php echo $data['search']['prm_acc_id']['value']; ?>" selected>
                            	<?php echo $data['search']['prm_acc_id']['text']; ?> 
                        	</option>
                    	<?php endif; ?>
                	</select>
				</div>
				...
				<tbody>
					...
					?>
								<tr>
									...
									<td width="7%"><?php echo $value['prm_entry_no']; ?></td>
									<td width="7%"><?php echo date('d-m-Y', strtotime($value['prm_entry_date'])); ?></td>
									<td width="41%"><?php echo strtoupper($value['account_name']); ?></td>
									<td width="7%"><?php echo round($value['prm_total_qty'], 0); ?></td>
									<td width="7%"><?php echo round($value['prm_sub_total'], 2); ?></td>
									<td width="7%"><?php echo round($value['prm_round_off'], 2); ?></td>
									<td width="7%"><?php echo round($value['prm_bill_disc'], 2); ?></td>
									<td width="7%"><?php echo round($value['prm_gst_amt'], 2); ?></td>
									<td width="7%"><?php echo round($value['prm_final_amt'], 2); ?></td>
								</tr>
					<?php 
							endforeach;
					?>
...
<script src="<?php echo assets('dist/js/report/purchase_return_summary.js')?>"></script>
	</body>
</html>