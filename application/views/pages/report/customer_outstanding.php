...
<script>
    let link 	= "report";
    let sub_link= "customer_outstanding";
</script>
<section class="container-fluid sticky_top">
	<form class="form-horizontal" id="search_form" action="<?php echo base_url('report/customer_outstanding?action=view')?>" method="get">
		<div class="d-flex justify-content-between">
			<nav aria-label="breadcrumb">
			  <ol class="breadcrumb">
			    <li class="breadcrumb-item active" aria-current="page">
			    	CUSTOMER OUTSTANDING(<span><i><?php echo $total_rows;?></i></span>)
			    </li>
			    ...
			    <li class="breadcrumb-item" aria-current="refresh-page">
			    	<a type="button" class="btn btn-sm btn-primary" onclick="redirectPage('report/customer_outstanding?action=view')" data-toggle="tooltip" data-placement="bottom" title="REFRESH"><i class="text-info fa fa-undo"></i></a>
			    </li>
			    <li class="breadcrumb-item" aria-current="print-page">
			    	<?php if(!empty($data['data'])): ?>
			    		<a target="_blank" type="button" class="btn btn-sm btn-primary mr-2" data-toggle="tooltip" data-placement="bottom" title="PDF" href="<?php echo base_url("report/customer_outstanding?submit=PDF&$url"); ?>">
			...
			<div class="d-flex flex-wrap justify-content-center floating-form">
				...
				<div class="d-flex col-6 col-sm-4 col-md-3 col-lg-3">
					<div class="floating-label">
						<input type="number" class="form-control floating-input" id="debit_frm" name="debit_frm" value="<?php echo $debit_frm ?>" placeholder=" " autocomplete="off"/>   
	                    <label for="inputEmail3">FROM SALES AMT</label>
					</div>
					<div class="floating-label">
						<input type="number" class="form-control floating-input" id="debit_to" name="debit_to" value="<?php echo $debit_to ?>" placeholder=" " autocomplete="off"/>   
	                    <label for="inputEmail3">TO SALES AMT</label>
					</div>
				</div>
				<div class="d-flex col-6 col-sm-4 col-md-3 col-lg-3">
					<div class="floating-label">
						<input type="number" class="form-control floating-input" id="debited_frm" name="debited_frm" value="<?php echo $debited_frm ?>" placeholder=" " autocomplete="off"/>   
	                    <label for="inputEmail3">FROM RECEIPT AMT</label>
					</div>
					<div class="floating-label">
						<input type="number" class="form-control floating-input" id="debited_to" name="debited_to" value="<?php echo $debited_to ?>" placeholder=" " autocomplete="off"/>   
	                    <label for="inputEmail3">TO RECEIPT AMT</label>
					</div>
				</div>
...
<?php $this->load->view('templates/footer'); ?>
	<script src="<?php echo assets('dist/js/report/customer_outstanding.js')?>"></script>
	</body>
</html>