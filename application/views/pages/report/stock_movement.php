<?php 
	...
	$diff_to 			= (isset($_GET['diff_to'])) ? $_GET['diff_to'] : "";
...
<script>
    let link 	= "report";
    let sub_link= "stock_movement";
</script>
<section class="container-fluid sticky_top">
	<form class="form-horizontal" id="search_form" action="<?php echo base_url('report/stock_movement?action=view')?>" method="get">
		<div class="d-flex justify-content-between">
			<nav aria-label="breadcrumb">
			  <ol class="breadcrumb">
			    <li class="breadcrumb-item active" aria-current="page">
			    	STOCK MOVEMENT(<span><i><?php echo $total_rows;?></i></span>)
			    </li>
			    ...
			    <li class="breadcrumb-item" aria-current="refresh-page">
			    	<a type="button" class="btn btn-sm btn-primary" onclick="redirectPage('report/stock_movement?action=view')" data-toggle="tooltip" data-placement="bottom" title="REFRESH"><i class="text-info fa fa-undo"></i></a>
			    </li>
			    <li class="breadcrumb-item" aria-current="print-page">
			    	<?php if(!empty($data['data'])): ?>
			    		<a target="_blank" type="button" class="btn btn-sm btn-primary mr-2" data-toggle="tooltip" data-placement="bottom" title="PDF" href="<?php echo base_url("report/stock_movement?submit=PDF&$url"); ?>">
			...
			<div class="d-flex flex-wrap justify-content-center floating-form">
				...
				<div class="d-flex mt-2 col-6 col-sm-4 col-md-3 col-lg-3">
					<div class="floating-label">
						<input type="text" class="form-control floating-input datepicker" id="date_frm" name="date_frm" value="<?php echo $date_frm ?>" placeholder=" " autocomplete="off" onchange="trigger_search()"/>   
	                    <label for="inputEmail3">FRM DATE</label>
					</div>
					<div class="floating-label">
						<input type="text" class="form-control floating-input datepicker" id="date_to" name="date_to" value="<?php echo $date_to ?>" placeholder=" " autocomplete="off" onchange="trigger_search()"/>   
	                    <label for="inputEmail3">TO DATE</label>
					</div>
				</div>
				<div class="d-flex mt-2 col-6 col-sm-4 col-md-3 col-lg-3">
					<div class="floating-label">
						<input type="number" class="form-control floating-input" id="diff_frm" name="diff_frm" value="<?php echo $diff_frm ?>" placeholder=" " autocomplete="off"/>   
	                    <label for="inputEmail3">DATE DIFF FRM</label>
					</div>
					<div class="floating-label">
						<input type="number" class="form-control floating-input" id="diff_to" name="diff_to" value="<?php echo $diff_to ?>" placeholder=" " autocomplete="off"/>
	                    <label for="inputEmail3">DATE DIFF TO</label>
					</div>
				</div>
				<div class="d-flex mt-2 col-6 col-sm-4 col-md-3 col-lg-3">
					<div class="floating-label">
				...
				<table class="table table-sm table-dark">
					<thead>
						<tr>
		                    ...
		                    <th width="7%">PUR RET. QTY</th>
		                    <th width="5%">SALE QTY</th>
		                    <th width="7%">SALE RET QTY</th>
		                    <th width="5%">PUR RATE</th>
		                    <th width="7%">BALANCE QTY</th>
		                    <th width="7%">STOCK AMT</th>
			            </tr>
					</thead>
					<tbody>
				...
				<tbody>
					...
					?>
								<tr>
									...
									<td width="8%"><?php echo $value['age_name']; ?></td>
									<td width="8%"><?php echo $value['pm_entry_date']; ?></td>
									<td width="7%"><?php echo $value['date_diff']; ?></td>
									<td width="5%"><?php echo $value['pt_qty']; ?></td>
									<td width="7%"><?php echo $value['prt_qty']; ?></td>
									<td width="5%"><?php echo $value['st_qty']; ?></td>
									<td width="7%"><?php echo $value['srt_qty']; ?></td>
									<td width="5%"><?php echo round($value['pt_rate'], 2); ?></td>
									<td width="7%"><?php echo $value['bal_qty']; ?></td>
									<td width="7%"><?php echo round($value['bal_amt'],2); ?></td>
...
<?php $this->load->view('templates/footer'); ?>
	<script src="<?php echo assets('dist/js/report/stock_movement.js')?>"></script>
	</body>
</html>