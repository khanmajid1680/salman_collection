<?php 
	...
	$credit_frm 	= (isset($_GET['credit_frm'])) ? $_GET['credit_frm'] : "";
	$credit_to 		= (isset($_GET['credit_to'])) ? $_GET['credit_to'] : "";
	$credited_frm 	= (isset($_GET['credited_frm'])) ? $_GET['credited_frm'] : "";
	$credited_to 	= (isset($_GET['credited_to'])) ? $_GET['credited_to'] : "";
	$bal_frm 		= (isset($_GET['bal_frm'])) ? $_GET['bal_frm'] : 1;
...
<script>
    let link 	= "report";
    let sub_link= "supplier_outstanding";
</script>
<section class="container-fluid sticky_top">
	<form class="form-horizontal" id="search_form" action="<?php echo base_url('report/supplier_outstanding?action=view')?>" method="get">
		<div class="d-flex justify-content-between">
			<nav aria-label="breadcrumb">
			  <ol class="breadcrumb">
			    <li class="breadcrumb-item active" aria-current="page">
			    	SUPPLIER OUTSTANDING(<span><i><?php echo $total_rows;?></i></span>)
			    </li>
			    ...
			    <li class="breadcrumb-item" aria-current="refresh-page">
			    	<a type="button" class="btn btn-sm btn-primary" onclick="redirectPage('report/supplier_outstanding?action=view')" data-toggle="tooltip" data-placement="bottom" title="REFRESH"><i class="text-info fa fa-undo"></i></a>
			    </li>
			    <li class="breadcrumb-item" aria-current="print-page">
			    	<?php if(!empty($data['data'])): ?>
			    		<a target="_blank" type="button" class="btn btn-sm btn-primary mr-2" data-toggle="tooltip" data-placement="bottom" title="PDF" href="<?php echo base_url("report/supplier_outstanding?submit=PDF&$url"); ?>">
			...
			<div class="d-flex flex-wrap justify-content-center floating-form">
				...
				<div class="d-flex col-6 col-sm-4 col-md-3 col-lg-3">
					<div class="floating-label">
						<input type="number" class="form-control floating-input" id="credit_frm" name="credit_frm" value="<?php echo $credit_frm ?>" placeholder=" " autocomplete="off"/>   
	                    <label for="inputEmail3">FROM PUR AMT</label>
					</div>
					<div class="floating-label">
						<input type="number" class="form-control floating-input" id="credit_to" name="credit_to" value="<?php echo $credit_to ?>" placeholder=" " autocomplete="off"/>   
	                    <label for="inputEmail3">TO PUR AMT</label>
					</div>
				</div>
				<div class="d-flex col-6 col-sm-4 col-md-3 col-lg-3">
					<div class="floating-label">
						<input type="number" class="form-control floating-input" id="credited_frm" name="credited_frm" value="<?php echo $credited_frm ?>" placeholder=" " autocomplete="off"/>   
	                    <label for="inputEmail3">FROM PAYMENT AMT</label>
					</div>
					<div class="floating-label">
						<input type="number" class="form-control floating-input" id="credited_to" name="credited_to" value="<?php echo $credited_to ?>" placeholder=" " autocomplete="off"/>   
	                    <label for="inputEmail3">TO PAYMENT AMT</label>
					</div>
				</div>
				...
				<table class="table table-sm table-dark">
					<thead>
						<tr>
		                    ...
		                    <th width="10%">PURCHASE AMT</th>
		                    <th width="10%">PAYMENT AMT</th>
		                    <th width="10%">PUR. RETURN AMT</th>
					...
					<tbody>
						<tr style="font-size: 15px; font-weight: bold;">
		                    ...
		                    <td ><?php echo round($data['totals']['bal_amt'], 2).' '. $data['totals']['label']; ?></td>
			            </tr>
					</tbody>
				</table>
			</div>
				...
				<tbody>
					...
					?>
								<tr>
									...
									<td width="5%"><?php echo $value['account_drcr']; ?></td>
									...
									<td width="10%"><?php echo $value['bal_amt']; ?></td>
...
<?php $this->load->view('templates/footer'); ?>
	<script src="<?php echo assets('dist/js/report/supplier_outstanding.js')?>"></script>
	</body>
</html>