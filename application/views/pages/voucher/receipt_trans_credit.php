    ...
    <tbody id="receipt_material_wrapper">
        <?php foreach ($trans_data as $key => $value): ?>
            <tr id="rowid_<?php echo $value['vt_sm_id'] ?>" class="floating-form">
                ...
                <td class="floating-label">
                    <input type="number" class="form-control floating-input" id="vt_adjusted_amt_<?php echo $value['vt_sm_id'] ?>" value="<?php echo $value['sm_allocated_amt'] + $value['sm_allocated_round_off']; ?>" readonly />
                    <input type="hidden" name="vt_allocated_amt[]" id="vt_allocated_amt_<?php echo $value['vt_sm_id'] ?>" value="0" />
                    <input type="hidden" name="vt_allocated_round_off[]" id="vt_allocated_round_off_<?php echo $value['vt_sm_id'] ?>" value="0" />
                </td>
                <td class="floating-label">
                    <input type="number" class="form-control floating-input" name="vt_return_qty[]" id="vt_return_qty_<?php echo $value['vt_sm_id'] ?>" value="<?php echo $value['vt_return_qty']; ?>" readonly />
                </td>
                <td class="floating-label">
                    <input type="number" class="form-control floating-input" name="vt_return_amt[]" id="vt_return_amt_<?php echo $value['vt_sm_id'] ?>" value="<?php echo $value['vt_return_amt']; ?>"  readonly/>
                    <input type="hidden" name="vt_adjust_amt[]" id="vt_adjust_amt_<?php echo $value['vt_sm_id'] ?>" value="0" />
                    <input type="hidden" name="vt_adjust_round_off[]" id="vt_adjust_round_off_<?php echo $value['vt_sm_id'] ?>" value="0" />
                </td>
                <td class="floating-label">
                    <input type="number" class="form-control floating-input" id="vt_balance_amt_show_<?php echo $value['vt_sm_id'] ?>" value="<?php echo $value['vt_total_amt'] - $value['vt_return_amt']; ?>" readonly />
                </td>
            </tr>
        <?php endforeach; ?>
    </tbody>
