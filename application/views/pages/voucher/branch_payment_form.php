...
<section class="d-flex justify-content-between sticky_top neu_flat_primary breadcrumb_pagination">
    <nav aria-label="breadcrumb">
      <ol class="breadcrumb">
        <li class="breadcrumb-item"><a href="<?php echo base_url('voucher/branch_payment?action=view'); ?>">BRANCH PAYMENT</a></li>
        ...
        <?php if(empty($master_data)): ?>
            <li class="breadcrumb-item" aria-current="save-page">
                <button type="button" class="btn btn-sm btn-primary master_block_btn" onclick="add_update_payment(<?php echo empty($master_data) ? 0 : $master_data[0]['vm_id']; ?>)" data-toggle="tooltip" data-placement="bottom" title="SAVE" tabindex="8" <?php echo empty($master_data) ? 'disabled="disabled"' : '' ?>><i class="text-success fa fa-save"></i></button>
            </li>
        <?php endif; ?>
        <li class="breadcrumb-item" aria-current="cancel-page">
            <a type="button" class="btn btn-sm btn-primary" href="<?php echo base_url('voucher/branch_payment?action=view')?>" data-toggle="tooltip" data-placement="bottom" title="CANCEL" tabindex="9"><i class="text-danger fa fa-close"></i></a>
        </li>
      </ol>
    </nav>
</section>
					...
					<div class="card-body">
						<div class="d-flex flex-wrap mt-2 form-group floating-form">
                            <div class="col-sm-12 col-md-4 col-lg-2 floating-label">
                                ...
                                <input type="hidden" name="vm_type" value="PAYMENT">
                            ...
                            <div class="col-sm-12 col-md-3 col-lg-3 floating-label bank_area <?php echo $bank_area; ?>">
                                <input type="text" class="form-control floating-input" id="vm_cheque_no" name="vm_cheque_no" value="<?php echo empty($master_data) ? '' : $master_data[0]['vm_cheque_no'] ?>" placeholder=" " autocomplete="off" tabindex="3"/>
                                <label for="inputEmail3">CHEQUE NO.</label>
                                <!-- <small class="form-text text-muted helper-text" id="vm_cheque_no_msg"></small> -->
                            </div>
                            <div class="col-sm-12 col-md-3 col-lg-3 floating-label bank_area <?php echo $bank_area; ?>">
                                <input type="text" class="form-control floating-input future-datepicker" id="vm_cheque_date" name="vm_cheque_date" value="<?php echo !empty($master_data) && $master_data[0]['vm_cheque_date'] != '0000-00-00' ? date('d-m-Y', strtotime($master_data[0]['vm_cheque_date'])) : ''; ?>" placeholder=" " autocomplete="off" tabindex="4"/>
					...
					<div class="card-body">
						<div class="d-flex flex-wrap mt-2 form-group floating-form">
                            <div class="col-sm-12 col-md-6 col-lg-6 floating-label">
                                ...
                                <input type="hidden" class="form-control floating-input" id="vm_bill_amt" name="vm_bill_amt" value="<?php echo empty($master_data) ? 0 : $master_data[0]['vm_bill_amt'] ?>" placeholder=" " readonly="readonly" />
                                <label for="inputEmail3">NOTES</label>
                            </div>
                            <div class="col-sm-12 col-md-6 col-lg-3 floating-label">
                                <input type="number" class="form-control floating-input" id="vm_total_amt" name="vm_total_amt" value="<?php echo empty($master_data) ? 0 : $master_data[0]['vm_total_amt'] ?>" placeholder=" " <?php echo empty($master_data) ? 'autocomplete="off"' : 'readonly' ?> />
                                <input type="hidden" id="vm_round_off" name="vm_round_off" value="<?php echo empty($master_data) ? 0 : $master_data[0]['vm_round_off'] ?>"/>
                                <label for="inputEmail3">PAYMENT AMT</label>
                            ...
                            <div class="col-sm-12 col-md-6 col-lg-3 floating-label">
                                ...
                                <?php else: ?>
                                    ...
                                    <label for="inputEmail3">GRN AMT</label>
                                <?php endif; ?>
                            ...
                            <?php if(!empty($trans_data)): ?>
                                ...
                                <tbody id="payment_material_wrapper">
                                    <?php foreach ($trans_data as $key => $value): ?>
                                        <tr id="rowid_<?php echo $value['vt_gm_id'] ?>" class="floating-form">
                                            <td width="5%" class="floating-label">
                                                <input type="hidden" id="vt_id_<?php echo $value['vt_gm_id'] ?>" name="vt_id[]" value="<?php echo $value['vt_id']; ?>" />
                                                <input type="hidden" id="vt_bill_clear_<?php echo $value['vt_gm_id'] ?>" name="vt_bill_clear[]" value="YES" />
                                                <input type="hidden" id="vt_gm_id_<?php echo $value['vt_gm_id'] ?>" name="vt_gm_id[]" value="<?php echo $value['vt_gm_id'] ?>" />
                                                <input type="text" class="form-control floating-input" name="vt_bill_no[]" id="vt_bill_no_<?php echo $value['vt_gm_id'] ?>" value="<?php echo $value['vt_bill_no']; ?>" readonly />
                                            </td>
                                            <td width="7%" class="floating-label">
                                                <input type="text" class="form-control floating-input" name="vt_bill_date[]" id="vt_bill_date_<?php echo $value['vt_gm_id'] ?>" value="<?php echo $value['vt_bill_date']; ?>" readonly />
                                            </td>
                                            <td width="6%" class="floating-label">
                                                <input type="number" class="form-control floating-input" name="vt_total_qty[]" id="vt_total_qty_<?php echo $value['vt_gm_id'] ?>" value="<?php echo $value['vt_total_qty']; ?>" readonly />
                                            </td>
                                            <td width="6%" class="floating-label">
                                                <input type="number" class="form-control floating-input" name="vt_total_amt[]" id="vt_total_amt_<?php echo $value['vt_gm_id'] ?>" value="<?php echo $value['vt_total_amt']; ?>" readonly />
                                                <input type="hidden" class="form-control floating-input" name="vt_allocated_amt[]" id="vt_allocated_amt_<?php echo $value['vt_gm_id'] ?>" value="<?php echo $value['vt_allocated_amt']; ?>" readonly />
                                                <input type="hidden" name="vt_allocated_round_off[]" id="vt_allocated_round_off_<?php echo $value['vt_gm_id'] ?>" value="<?php echo $value['vt_allocated_round_off']; ?>" />
                                            </td>
                                            <td width="8%" class="floating-label">
                                                <input type="number" class="form-control floating-input" name="vt_adjust_amt[]" id="vt_adjust_amt_<?php echo $value['vt_gm_id'] ?>" value="<?php echo $value['vt_adjust_amt']; ?>" readonly />
                                                <input type="hidden" name="vt_adjust_round_off[]" id="vt_adjust_round_off_<?php echo $value['vt_gm_id'] ?>" value="<?php echo $value['vt_adjust_round_off']; ?>" />
                                            </td>
                                        </tr>
                                    <?php endforeach; ?>
                                </tbody>
...
<script src="<?php echo assets('dist/js/voucher/branch_payment.js?v=1')?>"></script>
</body>
</html>