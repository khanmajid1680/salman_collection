...
<section class="container-fluid sticky_top">
	<form class="form-horizontal" id="search_form" action="<?php echo base_url('purchase_return?action=view')?>" method="get">
		<div class="d-flex justify-content-between">
			<nav aria-label="breadcrumb">
			  <ol class="breadcrumb">
			    <li class="breadcrumb-item active" aria-current="page">
			    	PURCHASE RETURN(<span id="count_reload"><i id="total_rows"><?php echo $total_rows;?></i></span>)
			    </li>
			    <li class="breadcrumb-item" aria-current="add-page">
			    	<a type="button" class="btn btn-sm btn-primary" onclick="redirectPage('purchase_return?action=add')" data-toggle="tooltip" data-placement="bottom" title="ADD NEW"><i class="text-success fa fa-plus"></i></a >
			    </li>
			    ...
			    <li class="breadcrumb-item" aria-current="reload-page">
			    	<a type="button" class="btn btn-sm btn-primary" onclick="redirectPage('purchase_return?action=view')" data-toggle="tooltip" data-placement="bottom" title="REFRESH"><i class="text-info fa fa-undo"></i></a>
			    </li>
					...
					<?php 
						if(!empty($data['data'])): 
							foreach ($data['data'] as $key => $value):
                    			$id = encrypt_decrypt("encrypt", $value['prm_id'], SECRET_KEY);
					?>

								<tr>
									<td width="3%"><?php echo $key+1; ?></td>
									<td width="10%"><?php echo $value['prm_entry_no']; ?></td>
									<td width="12%"><?php echo date('d-m-Y', strtotime($value['prm_entry_date'])); ?></td>
									<td width="20%"><?php echo strtoupper($value['account_name']); ?></td>
									<td width="10%"><?php echo $value['prm_total_qty']; ?></td>
									<td width="10%"><?php echo $value['prm_final_amt']; ?></td>
									<!-- <td width="3%">
										<a type="button" class="btn btn-sm btn-primary" target="_blank" href="<?php //echo base_url('purchase_return?action=print&id='.$value['prm_id']) ?>">
											<i class="text-info fa fa-print"></i>
										</a>										
									</td> -->
									<td width="3%">
										<a type="button" class="btn btn-sm btn-primary" onclick="redirectPage('<?php echo 'purchase_return?action=edit&id='.$value['prm_id'] ?>')">
									...
									<td width="4%">
										...
										<?php else: ?>
											<a type="button" class="btn btn-sm btn-primary" onclick="remove_master('<?php echo 'purchase_return/remove/'.$value['prm_id']?>');">
