	...
	class Purchasemdl extends CI_model{
		...
		public function get_data($wantCount, $per_page = PER_PAGE, $offset = OFFSET){
            ...
            if(isset($_GET['entry_date']) && !empty($_GET['entry_date'])){
                $subsql .= " AND pm.pm_entry_date ='".date('Y-m-d', strtotime($_GET['entry_date']))."'";
            }
            ...
            if(isset($_GET['bill_date']) && !empty($_GET['bill_date'])){
                $subsql .= " AND pm.pm_bill_date ='".date('Y-m-d', strtotime($_GET['bill_date']))."'";
            }
            ...
            if((isset($_GET['from_qty']) && !empty($_GET['from_qty'])) && (isset($_GET['to_qty']) && !empty($_GET['to_qty']))){
                $subsql .=" AND pm.pm_total_qty >= ".$_GET['from_qty'];
                $subsql .=" AND pm.pm_total_qty <= ".$_GET['to_qty'];
            }
            if((isset($_GET['from_bill_amt']) && !empty($_GET['from_bill_amt'])) && (isset($_GET['to_bill_amt']) && !empty($_GET['to_bill_amt']))){
		...
		public function get_data_for_edit($pm_id){
            ...
            $trans_query ="
                            ...
                            LEFT JOIN age_master age ON(age.age_id = pt.pt_age_id)
        ...
        public function get_select2_acc_id(){
            ...
            $query ="
                        SELECT acc.account_id as id, CONCAT(UPPER(acc.account_code),' - ', UPPER(acc.account_name)) as name
                        FROM ".$this->master." pm
