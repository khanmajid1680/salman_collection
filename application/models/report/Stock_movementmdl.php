<?php defined('BASEPATH') OR exit('No direct script access allowed');
	class Stock_movementmdl extends CI_model{
		...
		public function get_data(){
			...
			if(isset($_GET['date_frm'])){
				if($_GET['date_frm'] != ''){
					$subsql .=" AND pm.pm_entry_date >= '".date('Y-m-d', strtotime($_GET['date_frm']))."'";
				}
			}
			if(isset($_GET['date_to'])){
				if($_GET['date_to'] != ''){
					$subsql .=" AND pm.pm_entry_date <= '".date('Y-m-d', strtotime($_GET['date_to']))."'";
				}
			}
			if(isset($_GET['diff_frm'])){
				if($_GET['diff_frm'] != ''){
					$having .=" AND date_diff >= ".$_GET['diff_frm'];
				}
			}
			if(isset($_GET['diff_to'])){
				if($_GET['diff_to'] != ''){
					$having .=" AND date_diff <= ".$_GET['diff_to'];
				}
			}
			...
			$query 	="
						...
						DATE_FORMAT(pm.pm_entry_date, '%d-%m-%Y') as pm_entry_date, DATEDIFF(CURDATE(), pm.pm_entry_date) as date_diff
						...
						AND bm.bm_pm_id != 0
						$subsql
						GROUP BY acc.account_id, style.style_id, design.design_id, brand.brand_id, age.age_id, pm.pm_entry_date, pt_rate
						HAVING bal_qty > 0
						$having
						ORDER BY date_diff DESC, pt_rate DESC
					 ";
