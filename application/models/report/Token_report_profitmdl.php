<?php defined('BASEPATH') OR exit('No direct script access allowed');
	class token_report_profitmdl extends CI_model{
		...
		public function get_data($flag = false, $date_from = '', $date_to = ''){
			...
			$query ="
						...
						bm.bm_token_amt,
						st.st_qty,
						st.st_pt_rate as pt_amt, 
						st.st_rate st_rate, 
						st.st_disc_amt as st_disc, 
						st.st_sub_total_amt as st_amt,
						(st.st_sub_total_amt - st.st_pt_rate) as profit
						...
						AND bm.bm_token_check=1
						AND sm.sm_sales_type=0
						$subsql
						GROUP BY st.st_id DESC
						HAVING 1
						$having";
			// echo "<pre>"; print_r($query); exit;
			...
			$record['totals']['profit_loss']= $profit - ($expense + $srt_amt);
			// echo "<pre>"; print_r($record); exit;
