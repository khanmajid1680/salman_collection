<?php defined('BASEPATH') OR exit('No direct script access allowed');
	class BranchPaymentmdl extends CI_model{
		protected $master;
		protected $trans;
        ...
        public function get_branch_balance($data){
            $balance_amt = 0;
            $type        = TO_PAY;
            if(!empty($data)){
                $opening_amt        = 0;
                
                $grn_amt            = $this->get_grn_amt($data[0]['branch_id']);
                $payment_amt        = $this->get_payment_amt($data[0]['branch_id']);
                
                $receipt_amt        = $this->get_receipt_amt($data[0]['branch_id']);
                
                $closing_amt        = ($opening_amt + $grn_amt + $receipt_amt) - ($payment_amt);
        ...
        public function get_grn_amt($id){
            $query="
                        SELECT SUM(gm_final_amt) as amt
                        FROM grn_master
                        WHERE gm_branch = $id
                        AND gm_branch_id = ".$_SESSION['user_branch_id']."
                        GROUP BY gm_branch
                    ";
            // echo "<pre>"; print_r($query);exit();
        ...
        public function get_receipt_amt($id){
            $query="
                        SELECT SUM(vm_total_amt) as amt
                        FROM voucher_master
                        WHERE vm_branch = $id
        ...
        public function get_grn_balance($branch_id){
            $query = "
                        SELECT SUM(gm.gm_final_amt - (gm.gm_allocated_amt + gm.gm_allocated_round_off)) as amt
                        FROM grn_master gm 
                        WHERE gm.gm_branch = $branch_id
                        AND gm.gm_branch_id = ".$_SESSION['user_branch_id']."
                        GROUP BY gm.gm_branch
                    ";
            $data = $this->db->query($query)->result_array();
            return !empty($data) ? $data[0]['amt'] : 0;
        }
        public function get_data_for_payment($branch_id){
            $query = "
                        SELECT gm.*,(gm.gm_final_amt - (gm.gm_allocated_amt + gm.gm_allocated_round_off)) as bal_amt,
                            DATE_FORMAT(gm.gm_entry_date,'%d-%m-%Y') AS gm_entry_date
                        FROM grn_master gm 
                        WHERE gm.gm_branch = $branch_id 
                        AND gm_branch_id = ".$_SESSION['user_branch_id']."
                        HAVING bal_amt > 0
        ...
        public function _branch_id(){
            ...
            $query ="
                        ...
                        WHERE (gm.gm_final_amt - (gm.gm_allocated_amt + gm.gm_allocated_round_off)) > 0
                        AND gm.gm_branch_id = ".$_SESSION['user_branch_id']."
                        $subsql
            ...
            return [0 => ['id' => 0, 'name' => 'No pending grn available.']];
        }
	}
?>